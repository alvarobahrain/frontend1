...
function readActivities() {
    async function fetchAsync() {
        ...
        const response1 = await fetch('http://localhost:3000/activities/9');
        ...
        txt1+= '<h5 class="card-title">Data de Fim:</h5><p class="card-text">' +actDouro.data_fim + '/p> <br><br>'
        //txt1+= '<input type="submit" id="btnDouro" class="btn_2" value="submeter"></div></div> <br';
        txt1+= '</div></div><br>'

        const response2 = await fetch('http://localhost:3000/activities/10');
        ...
        txt1+=' <h5 class="card-title">Data de Inicio:</h5><p class="card-text">' + actCavado.data_inicio +'</p>';
        txt1+= '<h5 class="card-title">Data de Fim:</h5><p class="card-text">' +actCavado.data_fim + '/p> <br><br>'
        //txt1+= '<button type="submit" id="btnCavado" class="btn_2">Submeter</button></div></div> <br';
        txt1+= '</div></div><br>'

        const response3 = await fetch('http://localhost:3000/activities/11');
        ...
        txt1+= '<h5 class="card-title">Data de Fim:</h5><p class="card-text">' +actAve.data_fim + '/p> <br><br>'
        //txt1+= '<button type ="submit" id="btnAve" class="btn_2">Submeter</button></div></div> <br';
        txt1+= '</div></div><br>'

        txt2+='<option value="9">'+ actDouro.nome_atividade + '</option>';
        txt2+='<option value="10">'+ actCavado.nome_atividade + '</option>';
        txt2+='<option value="11">'+ actAve.nome_atividade + '</option>';
