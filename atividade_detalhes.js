...
window.onload = () => {
    ...
    if(txtName=="" || displayOption=="escolher") {
        ...
        let response;
        if(isNew) {
                response = await fetch('http://localhost:3000/teams', {
                headers: {
    ...
    if(txtEmailUser=="") {
        ...
        let response1;
        if(isNew) {
            response1= await fetch('http://localhost:3000/participants');
            ...
            response2 = await fetch(`http://localhost:3000/teams/${newTeamID}/participants/${participantID}`, {
                headers: {
                    "Content-Type": "application/x-www-form-urlencoded"
                },
                method: "POST",
                ...
                if(!response.ok) {
                    ...
                    if(cont==0) {
                        fetch(`http://localhost:3000/teams/${newTeamID}/participants/${IdUSER}`, {
                            headers: {
                                "Content-Type": "application/x-www-form-urlencoded"
                            },
                            method: "POST",
    ...
    btnCap.addEventListener("click", async (event) => {
        ...
        if(txtEmailCap=="") {
            ...
            const response4 = await fetch(`http://localhost:3000/teams/${newTeamID}/participants`);
            const useers = await response4.json();

            for(const useer of useers) {
                if(useer.Id_Participante==txtEmailCap) {
                fetch(`http://localhost:3000/teams/${newTeamID}/participants/${txtEmailCap}`, {
                    headers: {
                        "Content-Type": "application/x-www-form-urlencoded"
                    },
                    method: "PUT",
    ...
    const readPartInTeam = async () => {
        ...
        const idUSER = getCookie('id');
        let txt="";
        const response5 = await fetch(`http://localhost:3000/teams/${newTeamID}/participants`);
